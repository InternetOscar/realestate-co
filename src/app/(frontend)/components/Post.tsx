...
export default function Post({ data }: { data: Article }) {
  ...
  // TODO: replace with payload equivalent
  ...
  for (let i = 0; i < fullDescription.length; i++) {
    if (fullDescription[i].children[0]) {
    ...
    <article className=" m-[20px]">
      <div className="text-center h-[700px] grid grid-cols-4 border-b border-black mb-[20px]">
        ...
        <div className="m-auto col-span-2">
          <h1 className="leading-tight text-8xl italic font-display font-light block">
            {streetAddress}
          </h1>
          <h2 className="leading-tight text-8xl font-display font-light">{suburb}</h2>
          <p className="uppercase m-8">
            {nearestCity}, {state}
          </p>
          <div>
      ...
      <div className="flex w-full h-[950px] object-cover overflow-hidden rounded-lg items-center">
        {imageUrl && (
          <Image
            src={imageUrl}
            alt={title ? title : 'article cover image'}
            width={1500}
            height={1200}
      ...
      <div className="grid grid-cols-4">
        <div className="mt-[57px] mr-[60px] sticky top-[80px]">
          ...
          <div className="mt-8">
            <span className="border px-10 py-2 uppercase rounded-md hover:border-zinc-500 transition-all cursor-grab active:cursor-grabbing">
              Enquire Now
            </span>
          </div>
        </div>
        <p className="text-5xl font-display font-light col-start-2 col-end-5 mt-[60px]">
          {description}
        </p>
        <div
          className="col-start-2 col-end-4 text-2xl font-display font-light mt-[-70px] *:pb-5"
          // eslint-disable-next-line react/no-danger -- content is sanitized server-side
          dangerouslySetInnerHTML={{ __html: fixedDescription }}
        ></div>
        <div className="flex flex-nowrap overflow-auto col-start-2 col-end-5 my-[50px] mr-[-20px]">
          {/* {data.attributes.blocks.map((section: any, index: number) => postRenderer(section, index))} */}
        </div>
        <p className="col-start-2 col-end-4 text-2xl font-display font-light mb-4 pb-8">
          Mauris convallis turpis nec tellus vestibulum, in ornare purus euismod. Suspendisse
          potenti. Proin vulputate ultrices urna, vel aliquam nunc convallis vel. Aliquam et sem sed
          nunc congue bibendum. Ut maximus fringilla cursus. Proin a diam sodales, hendrerit risus
          ut, semper mauris. Proin ut placerat est, quis tincidunt orci. Integer tempus justo purus.
          Integer vel metus blandit, posuere urna in, luctus massa. Nulla facilisi. Nunc
          sollicitudin quis odio quis efficitur. Aliquam sit amet lobortis velit. In accumsan leo
          vitae ultrices semper. Pellentesque leo arcu, tincidunt vitae lacus nec, convallis
          scelerisque tortor. Integer ac facilisis massa, ut dictum nunc.
        </p>
